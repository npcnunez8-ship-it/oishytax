...
import { Cloud, Droplets, Thermometer, Plus, Trash2, MapPin, Wallet, ArrowLeft, ChevronRight, ChevronDown, TrendingUp, TrendingDown, Sprout, Package, FlaskConical, User, Award, Medal, Trophy, Star, Lock, Zap, Globe, Users, Search, Phone, UserPlus, Loader2, Camera, Upload, ScanLine, X, FileText, CheckCircle, AlertTriangle, MessageCircle, Send, Bot, Megaphone } from "lucide-react";
...
// --- HELPER: ULTRA COMPRESSOR (The Magic Fix) ---
  ...
  return new Promise((resolve, reject) => {
    ...
    reader.onload = (event) => {
      ...
      img.onload = () => {
        ...
        // Resize to 300px (Tiny but enough for AI to see disease)
        const MAX_WIDTH = 300; 
        ...
        // Convert to Base64 JPEG at 50% quality
...
// 1. CHAT (Groq Llama 3.3 - Keeps working)
...
// 2. VISION (Pollinations AI - Fixed with Compression)
const callAiVision = async (file: File, lang: "bn" | "en") => {
  try {
    // A. Compress Image (Crucial Step)
    const base64Url = await compressImage(file); 
    
    const promptText = `Analyze this crop image.
    1. Identify the crop.
    2. Is it Healthy or Diseased?
    3. If diseased, give the disease name.
    4. Give 2 short treatments.
    
    Output JSON ONLY: { "status": "Healthy/Diseased", "diagnosis": "Details...", "advice": ["Tip 1", "Tip 2"] }
    Translate to ${lang === 'bn' ? 'Bangla' : 'English'}.`;

    // B. Call Pollinations (Supports CORS + Vision)
    const response = await fetch("https://text.pollinations.ai/", {
      ...
      body: JSON.stringify({
        ...
        model: "openai", // Uses GPT-4o-mini (Best Vision)
    ...
    return {
        ...
        color: isRotten ? "text-red-600" : "text-green-600",
        advice: resultJson.advice || ["Consult an expert."],
...
const TRANSLATIONS: any = {
  bn: {
    app_title: "কৃষি বন্ধু ২.০",
    sub_title: "কৃষকের হাসি",
    net_profit: "মোট লাভ",
    ...
    scanner_title: "ফসল স্ক্যানার",
    upload_photo: "ছবি আপলোড",
    take_photo: "ছবি তুলুন",
    analyzing: "এআই বিশ্লেষণ করছে...",
    result_fresh: "ফসলটি সুস্থ (Fresh)",
    result_rotten: "রোগ সনাক্ত হয়েছে (Disease Detected)",
    advice_title: "পরামর্শ ও করণীয়:",
    chat_title: "সমাধান চ্যাটবক্স",
    chat_placeholder: "আপনার সমস্যা লিখুন...",
    chat_welcome: "স্বাগতম! আমি আপনার কৃষি সহকারী। আপনার ফসল বা চাষাবাদ নিয়ে যেকোনো প্রশ্ন করুন।",
    chat_typing: "এআই ভাবছে...",
  },
  en: {
    app_title: "KrishiBondhu 2.0",
    sub_title: "Krishoker Hasi",
    ...
    scanner_title: "Crop Scanner",
    upload_photo: "Upload Photo",
    take_photo: "Take Photo",
    analyzing: "AI Analysis in progress...",
    result_fresh: "Crop is Healthy (Fresh)",
    result_rotten: "Disease Detected (Rotten)",
    advice_title: "AI Diagnosis & Plan:",
    chat_title: "Solution Chatbox",
    chat_placeholder: "Ask your problem...",
    chat_welcome: "Welcome! I am your Agri-Assistant. I use Real AI to answer your questions perfectly.",
...
const Dashboard = () => {
  ...
  // --- SMART ALERT SYSTEM LOGIC ---
  const [alertLevel, setAlertLevel] = useState<"Normal" | "Warning" | "Critical">("Normal");
  const [alertMessage, setAlertMessage] = useState({ bn: "", en: "" });
  ...
  useEffect(() => {
    ...
    let msgBn = "আবহাওয়া অনুকূল। নিয়মিত পর্যবেক্ষণ চালিয়ে যান।";
    let msgEn = "Weather is favorable. Continue routine monitoring.";

    // DECISION ENGINE RULES
    if (rain >= 50 && humidity >= 80) {
        level = "Critical";
        msgBn = "আগামীকাল বৃষ্টি হবে এবং আপনার আলু গুদামে আর্দ্রতা বেশি। এখনই ফ্যান চালু করুন।";
        msgEn = "Tomorrow it will rain and humidity is high in potato storage. Turn on fans now.";
    } 
    else if (temp >= 36) {
        level = "Warning";
        msgBn = "তাপমাত্রা অসহনীয় পর্যায়ে (৩৬°C+)। ফসলের ক্ষতি এড়াতে বিকেলে সেচ দিন।";
        msgEn = "Temperature is critical (36°C+). Irrigate in the afternoon to avoid crop damage.";
    }
    else if (rain >= 70) {
        level = "Warning";
        msgBn = "ভারী বৃষ্টির সম্ভাবনা। দয়া করে সার প্রয়োগ বন্ধ রাখুন এবং ড্রেনেজ চেক করুন।";
        msgEn = "Heavy rain expected. Stop fertilizer application and check drainage.";
    }

    setAlertLevel(level);
    setAlertMessage({ bn: msgBn, en: msgEn });

    // Simulate SMS in Console
    if (level === "Critical") {
        console.log(`%c[SMS ALERT SYSTEM]: ${msgBn}`, "color: red; font-weight: bold; font-size: 14px; background: #ffebeb; border: 1px solid red; padding: 4px; border-radius: 4px;");
    }
  ...
  const handleFileUpload = async (event: any) => {
      ...
      if (file) {
          ...
          reader.onloadend = async () => {
              ...
              // Use Real Gemini Vision AI
              let aiData = await callAiVision(file, lang);
              
              // NO FALLBACK - Show error if AI fails
              if (aiData.status === "Error" || aiData.status === "API Error" || aiData.status === "Analysis Failed" || aiData.status === "Key Missing" || aiData.status === "Warming Up") {
  ...
  return (
    <div className="min-h-screen bg-[#F5F7F5] font-['Hind_Siliguri'] pb-20">
      <header className="bg-white p-4 sticky top-0 z-10 shadow-sm flex justify-between items-center">
        ...
        <AnimatePresence>{showMenu && (<motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} className="absolute top-16 left-4 bg-white shadow-xl rounded-xl border border-green-100 p-2 z-50 w-56"><button onClick={() => { setShowMenu(false); setView("scanner"); }} className="flex items-center gap-3 w-full p-3 hover:bg-green-50 rounded-lg text-gray-700 font-bold border-b border-gray-100"><ScanLine size={18} className="text-[#2F5233]"/> {t.scanner_title}</button><button onClick={() => { setShowMenu(false); setView("chat"); }} className="flex items-center gap-3 w-full p-3 hover:bg-blue-50 rounded-lg text-gray-700 font-bold"><MessageCircle size={18} className="text-blue-600"/> {t.chat_title}</button></motion.div>)}</AnimatePresence>
      ...
      <div className="p-4 space-y-6 max-w-md mx-auto">
        ...
        {/* --- SMART ALERT BOX (NEW ADDITION) --- */}
        <div className={`p-4 rounded-2xl shadow-sm border relative overflow-hidden transition-all duration-300 ${alertLevel === 'Critical' ? 'bg-red-50 border-red-200' : alertLevel === 'Warning' ? 'bg-orange-50 border-orange-200' : 'bg-blue-50 border-blue-200'}`}>
            <div className="flex items-start gap-4 relative z-10">
                <div className={`p-3 rounded-full flex-shrink-0 ${alertLevel === 'Critical' ? 'bg-red-100 text-red-600 animate-pulse' : alertLevel === 'Warning' ? 'bg-orange-100 text-orange-600' : 'bg-blue-100 text-blue-600'}`}>
                    <Megaphone size={24} />
                </div>
                <div>
                    <h3 className={`font-bold text-lg mb-1 flex items-center gap-2 ${alertLevel === 'Critical' ? 'text-red-700' : alertLevel === 'Warning' ? 'text-orange-700' : 'text-blue-700'}`}>
                        {lang === 'bn' ? 'স্মার্ট অ্যালার্ট' : 'Smart Alert'}
                        {alertLevel === 'Critical' && <span className="text-xs bg-red-600 text-white px-2 py-0.5 rounded-full animate-pulse">Critical</span>}
                    </h3>
                    <p className="text-gray-700 font-medium leading-relaxed text-sm">
                        {lang === 'bn' ? alertMessage.bn : alertMessage.en}
                    </p>
                    {alertLevel === 'Critical' && (
                        <div className="mt-2 text-xs font-bold text-red-500 flex items-center gap-1 bg-red-100 px-2 py-1 rounded w-fit">
