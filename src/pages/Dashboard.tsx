...
import { 
  ...
  BarChart3, Menu, X, Speaker, Award, Plus, Loader2
} from 'lucide-react';
...
const TRANSLATIONS = {
  en: {
    ...
    scan_btn: "Upload Image",
    analyzing: "Analyzing with AI...",
    scan_result_pest: "Stem Borer",
    scan_result_action: "Use Neem oil spray and remove affected parts.",
    voice_listening: "Listening...",
    voice_query: "How is the weather today?",
    voice_response: "Sky is clear, temperature is 36 degrees.",
    stats: "Stats",
    profile: "Profile",
    active_batches: "Active Batches",
    no_crops: "No crops registered yet.",
    online: "Online",
    privacy_note: "Visualizing local risks anonymously"
  },
  bn: {
    title: "হারভেস্টগার্ড",
    ...
    scan_btn: "ছবি আপলোড করুন",
    analyzing: "এআই দ্বারা বিশ্লেষণ করা হচ্ছে...",
    scan_result_pest: "মাজরা পোকা",
    scan_result_action: "নিম তেলের স্প্রে ব্যবহার করুন এবং আক্রান্ত অংশ কেটে ফেলুন।",
    voice_listening: "শুনছি...",
    voice_query: "আজকের আবহাওয়া কেমন?",
    voice_response: "আজ আকাশ পরিষ্কার, তাপমাত্রা ৩৬ ডিগ্রি।",
    stats: "পরিসংখ্যান",
    profile: "প্রোফাইল",
    active_batches: "সক্রিয় ব্যাচ",
    no_crops: "কোন ফসল নিবন্ধিত নেই।",
    online: "অনলাইন",
    privacy_note: "গোপনীয়তা রক্ষা করে স্থানীয় ঝুঁকি দেখা হচ্ছে"
  }
};
...
const WeatherWidget = ({ lang }: { lang: Language }) => {
  ...
  const weather: WeatherData = { temp: 36, humidity: 85, rainChance: 85, location: 'Bogura' };
...
const RiskMap = ({ lang }: { lang: Language }) => {
  ...
  const getCropName = (pin: MapPin) => {
    if (lang === 'en') return pin.crop;
    if (pin.crop === 'Potato') return 'আলু';
    if (pin.crop === 'Paddy') return 'ধান';
    if (pin.crop === 'Rice') return 'চাল';
    if (pin.crop === 'Onion') return 'পেঁয়াজ';
    return pin.crop;
  };
            ...
            >
              <div className="flex justify-between">
                <span className="font-bold">
                  {lang === 'bn' ? 'ফসল: ' : 'Crop: '} 
                  {getCropName(pins.find(p => p.id === selectedPin)!)}
                </span>
              ...
              <div className={pins.find(p => p.id === selectedPin)?.risk === 'High' ? 'text-red-600' : 'text-green-600'}>
                {lang === 'bn' ? 'ঝুঁকি: ' : 'Risk: '}
                {(() => {
                    const risk = pins.find(p => p.id === selectedPin)?.risk;
                    if (lang === 'en') return risk;
                    return risk === 'High' ? 'উচ্চ' : risk === 'Medium' ? 'মাঝারি' : 'নিম্ন';
                })()}
              </div>
            </motion.div>
          )}
...
const PestScanner = ({ lang }: { lang: Language }) => {
  ...
  const handleScan = () => {
    ...
    setTimeout(() => {
      setAnalyzing(false);
      setResult({
        pest: t.scan_result_pest,
        risk: 'High',
        action: t.scan_result_action
      });
    }, 2000);
  };
  ...
  return (
    <div className="bg-white p-4 rounded-xl shadow-md border border-green-100">
      ...
      {!result ? (
        <div className="flex flex-col items-center gap-4 py-6 border-2 border-dashed border-gray-300 rounded-lg bg-gray-50 transition-colors hover:border-purple-300 hover:bg-purple-50">
          {analyzing ? (
            ...
            <>
              {/* Note: This is a simulation button. In a real app, this would be an input[type="file"] */}
              <button 
                onClick={handleScan} 
                className="bg-green-600 hover:bg-green-700 text-white px-4 py-2 rounded-lg flex items-center gap-2 shadow-md transition-all active:scale-95"
              >
                <Upload size={18} /> {t.scan_btn}
              </button>
        ...
        <div className="bg-red-50 p-4 rounded-lg border border-red-200">
          ...
          <button onClick={() => setResult(null)} className="text-xs text-blue-600 underline hover:text-blue-800">
            {lang === 'bn' ? 'আবার স্ক্যান করুন' : 'Scan Another'}
          </button>
        </div>
      )}
    </div>
  ...
  const toggleListen = () => {
    if (!isListening) {
      ...
      setTimeout(() => {
        ...
        const query = t.voice_query;
        setTranscript(query);
        
        // Simulate response delay
        setTimeout(() => {
          setResponse(t.voice_response);
        }, 800);
      }, 2000);
    } else {
      setIsListening(false);
  ...
  return (
    <div className="fixed bottom-20 right-4 z-50">
      ...
      <button 
        ...
        className={`p-4 rounded-full shadow-lg transition-all ${isListening ? 'bg-red-500 animate-pulse' : 'bg-green-700 hover:bg-green-800'}`}
      >
...
const LandingPage = ({ onStart, lang, setLang }: { onStart: () => void, lang: Language, setLang: (l: Language) => void }) => {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-green-900 to-green-700 text-white flex flex-col font-sans">
      <nav className="p-6 flex justify-between items-center">
        ...
        <button 
          ...
          className="bg-white/20 px-4 py-2 rounded-full backdrop-blur-sm hover:bg-white/30 transition font-medium"
        >
      ...
      <main className="flex-1 flex flex-col items-center justify-center text-center p-6 space-y-8">
        ...
        <div className="flex gap-4 items-center my-8 bg-white/10 p-6 rounded-xl backdrop-blur-md">
          ...
          <motion.div animate={{ color: ["#fff", "#ef4444", "#fff"] }} transition={{ repeat: Infinity, duration: 1.5 }} className="text-center">
            ...
            <span className="text-xs opacity-75">Warning</span>
          </motion.div>
          <div className="w-8 h-0.5 bg-white/30"></div>
...
const Dashboard: React.FC = () => {
  ...
  const addCrop = () => {
    ...
    const newCrop: CropBatch = {
      ...
      etcl: 72, // Prediction Engine Result (Mocked)
      riskLevel: 'Medium'
    };
    ...
    <div className="min-h-screen bg-gray-50 font-sans pb-24 text-gray-800">
      ...
      <div className="max-w-md mx-auto p-4 space-y-6">
        ...
        <div className="bg-gradient-to-r from-green-600 to-green-500 rounded-2xl p-6 text-white shadow-lg relative overflow-hidden">
          ...
          <div className="flex gap-2 mt-2 overflow-x-auto relative z-10 scrollbar-hide">
            <span className="bg-white/20 px-3 py-1 rounded-full text-xs backdrop-blur-sm whitespace-nowrap border border-white/10">🏅 {t.badge_1}</span>
            <span className="bg-white/20 px-3 py-1 rounded-full text-xs backdrop-blur-sm whitespace-nowrap border border-white/10">🛡️ {t.badge_2}</span>
          </div>
        </div>
        ...
        <div>
          <div className="flex justify-between items-center mb-4">
            <h3 className="font-bold text-gray-700">{t.active_batches}</h3>
            <button 
              onClick={addCrop} 
              className="bg-green-700 hover:bg-green-800 transition text-white px-3 py-1 rounded-lg text-sm flex items-center gap-1 shadow"
            >
          ...
          {crops.length === 0 && (
            <p className="text-center text-gray-400 py-8 bg-white rounded-xl border border-dashed border-gray-300">
              {t.no_crops}
            </p>
          )}
      ...
      {/* Bottom Nav */}
      <nav className="fixed bottom-0 w-full bg-white border-t border-gray-200 py-3 px-6 flex justify-between text-gray-400 z-40 shadow-[0_-4px_6px_-1px_rgba(0,0,0,0.05)]">
        <button className="flex flex-col items-center text-green-700 transition">
          <Leaf size={24} />
          <span className="text-[10px] font-bold mt-1">{t.nav_dashboard}</span>
        </button>
        <button className="flex flex-col items-center hover:text-green-600 transition">
          <BarChart3 size={24} />
          <span className="text-[10px] mt-1">{t.stats}</span>
        </button>
        <button className="flex flex-col items-center hover:text-green-600 transition">
          <User size={24} />
